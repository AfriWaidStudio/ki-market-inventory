...
CREATE TYPE public.market_inventory_ledger_entry_type AS ENUM (
  'paper_position_opened','paper_realized_profit','manual_capital_committed',
  'manual_realized_profit','fee_recorded','adjustment'
);
...
ALTER TABLE public.market_inventory_trades
  ...
  ADD CONSTRAINT market_inventory_trades_stage_check CHECK (
    stage IN ('paper_active','paper_closed','bought','awaiting_transfer','received',
      'listed_for_sale','awaiting_payment','ready_to_close','partially_closed','closed','cancelled')
  );

UPDATE public.market_inventory_trades
SET remaining_amount = CASE WHEN status = 'closed' THEN 0 ELSE amount END,
    ...
    stage = CASE WHEN status = 'closed' THEN 'closed' WHEN status = 'cancelled' THEN 'cancelled' ELSE 'bought' END
WHERE remaining_amount IS NULL;

ALTER TABLE public.market_inventory_trades ALTER COLUMN remaining_amount SET NOT NULL;
...
CREATE TABLE public.market_inventory_trade_events (
  ...
  event_type text NOT NULL, from_stage text, to_stage text, metadata jsonb,
...
CREATE TABLE public.market_inventory_trade_fees (
  ...
  currency text NOT NULL DEFAULT 'NGN', note text,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
INSERT INTO public.market_inventory_trade_events (user_id, trade_id, event_type, to_stage, metadata, created_at)
SELECT user_id, id, 'legacy_trade_imported', stage,
  jsonb_build_object('status', status, 'trade_type', trade_type), created_at
FROM public.market_inventory_trades;

INSERT INTO public.market_inventory_capital_ledger (user_id, trade_id, trade_type, entry_type, amount, currency, description, metadata, created_at)
SELECT user_id, id, trade_type, 'manual_capital_committed', amount * buy_price, currency,
  'Backfilled capital committed for existing manual trade',
  jsonb_build_object('amount', amount, 'buy_price', buy_price), buy_time
FROM public.market_inventory_trades WHERE trade_type = 'manual';

INSERT INTO public.market_inventory_capital_ledger (user_id, trade_id, trade_type, entry_type, amount, currency, description, metadata, created_at)
SELECT user_id, id, trade_type, 'manual_realized_profit', COALESCE(actual_profit, realized_profit, 0), currency,
  'Backfilled realized profit for existing closed manual trade',
  jsonb_build_object('source', 'legacy_actual_profit'), COALESCE(sell_time, updated_at)
FROM public.market_inventory_trades WHERE trade_type = 'manual' AND status = 'closed'
  AND COALESCE(actual_profit, realized_profit, 0) <> 0;

INSERT INTO public.market_inventory_capital_ledger (user_id, trade_id, trade_type, entry_type, amount, currency, description, metadata, created_at)
SELECT user_id, id, trade_type, 'fee_recorded', -ABS(COALESCE(final_fees, estimated_fees, 0)), currency,
  'Backfilled recorded fee for existing trade',
  jsonb_build_object('source', 'legacy_fee_columns'), COALESCE(sell_time, updated_at)
FROM public.market_inventory_trades WHERE COALESCE(final_fees, estimated_fees, 0) > 0;